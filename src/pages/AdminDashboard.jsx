...
export default function AdminDashboard() {
  ...
  const [ojt, setOjt] = useState({
    ...
    // DB saved rendered time excluding the currently-active (no time_out yet) segment.
    renderedSecondsBase: 0,
    // If the user is currently clocked-in today, this stores the start ms of the active segment.
  ...
  // Live-update the OJT cards while the user is currently clocked-in.
  useEffect(() => {
    if (userRole !== 'tla' || !ojt.clockedInStartMs) return;
    const id = setInterval(() => setOjtNowTick((t) => t + 1), 1000);
    return () => clearInterval(id);
  }, [userRole, ojt.clockedInStartMs]);

  // Periodically refetch base values so the live clock-out updates quickly.
  useEffect(() => {
    if (userRole !== 'tla' || !ojt.clockedInStartMs || !user?.id) return;
    const id = setInterval(() => fetchOjt(true), 15000);
    return () => clearInterval(id);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [userRole, ojt.clockedInStartMs, user?.id, supabase]);
  ...
  const fetchOjt = async (bypassCache = false) => {
    if (!user?.id) return;
    const cacheKey = `admin:ojt:v2:${user.id}`;
    if (!bypassCache) {
    ...
    try {
      ...
      // Live segment: if the last segment for today's log has no time_out, add elapsed time.
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-4 shadow-sm">
              ...
              <p className="mt-1 text-2xl font-semibold text-gray-900 dark:text-gray-100">
                {minutesToHours(renderedMinutesLive)}
              </p>
            </div>
            <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-4 shadow-sm">
